...
class Authentication:
    ...
    def update_user(self,username,hash_password,salt):
        ...
        update_query = f"UPDATE users SET hashed_password = ?, salt = ? WHERE username = ?;"
        user_values = (hash_password,salt,username)
        if not self.database.execute_query(update_query, user_values):
    ...
    def store_user(self,username, hashed_password,salt):
        ...
        insert_query = f"INSERT INTO users (username, hashed_password, salt) VALUES (?, ?, ?);"
        user_values = (username, hashed_password, salt)
        if not self.database.execute_query(insert_query,user_values):
            self.logger.log_error(f"Error in executing query {insert_query}")

    def retrieve_userInfo(self,username):

        select_query = f"SELECT hashed_password, salt FROM users WHERE username = ?;"
        user_values = (username,)
        if self.database.execute_query(select_query,user_values):
